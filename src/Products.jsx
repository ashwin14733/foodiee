...
const Products = ({cart, setCart }) => {
    ...
    // // ✅ Make sure productdetails is defined before filtering
    // const filteredproducts = productdetails ? productdetails.filter((items) =>
    //     (items?.product || '').toLowerCase().includes((searchitems || '').toLowerCase())
    // ) : [];


//     const filteredproducts = productdetails
//   ? productdetails.filter((items) =>
//       (items?.product || '')
//         .toLowerCase()
//         .replace(/\s+/g, '') // remove spaces
//         .includes((searchitems || '').toLowerCase().replace(/\s+/g, ''))
//     )
//   : [];

const filteredproducts = productdetails
  ? productdetails.filter((items) =>
      (items.product ?? "") // if product is undefined, make it empty string
        .toLowerCase()
...
console.log("all products:", productdetails.map(p => p.product));
    ...
    const addtocart = (product) => {
        setCart(cart => {
            ...
            return cartitems ? 
                cart.map((item) => item.id === product.id ? { ...item, count: item.count + 1 } : item) : 
                [...cart, { ...product, count: 1 }]
        })
    }

    const particularproduct = (element, numbers) => {
        const uniquedata = countdown.filter((object, index, arr) => 
            index == arr.findIndex((ddd) => ddd.id === object.id))
            ...
            {searchitems && (
                <div className="container mt-4">
                    <div className="alert alert-info">
                        <h5>Search Results for: "{searchitems}"</h5>
                    ...
                    {filteredproducts.length > 0 ? (
                                    ...
                                    <div className="card-body d-flex flex-column text-center">
                                        ...
                                        <div className="quantity">
                                            <button className='plusbtn' onClick={() => {
                                                setCountdown([...countdown, element]);
                                                particularproduct(element, -1)
                                            }}>-</button>
                                        ...
                                        <button className='btn btn-warning mt-auto' onClick={() => {
                                            addtocart(element);
                                            // ✅ Make sure Swal is imported or use regular alert
                        ...
                        <div className="col-12 text-center mt-5">
                            <div className="alert alert-warning">
                                <h4>No items found for "{searchitems}"</h4>
                                <p>Try searching with different keywords</p>
                            </div>
                        </div>
                    ) : (
                        // Show message when no search term and no products
                        <div className="col-12 text-center mt-5">
